...
public class displayingdata : MonoBehaviour
{
    ...
    private void Update()
    {
        if (femaleIdentify.text == "Female" && stageText.text != "Kid")
        {
            // Enable the two objects
            textStatus.SetActive(true);
            buttonStatus.SetActive(true);
        }
        else if (femaleIdentify.text == "Female" && stageText.text == "Kid")
        {
            textStatus.SetActive(true);
...
public void ImageChangeStage(){
    {
        ...
        if (stageText.text != null)
        {
            ...
            if ( stageText.text== "Buckling" || stageText.text == "Buck")
            {
                selectedPath = pathA;
            }
...
void LoadAndDisplayConditionalImage(DataArchived targetItem)
    {
        ...
        {
            // Determine which path to use based on some conditions
            if (targetItem.stageG == "Buckling" || targetItem.stageG == "Buck")
            {
                selectedPath = pathA;
            }
