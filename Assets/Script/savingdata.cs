...
public class savingdata : MonoBehaviour
{
    ...
    public Text obtainText;
     // for displaying data and refresh ui 
    ...
    {   
        ...
        LoadData();
        LoadWeightData();
        // Check if the targetAge exists in the JSON data
    int targetAge = PlayerPrefs.GetInt("AgePass");
    bool ageExists = dataList.Exists(goatData => goatData.age == targetAge);
    
    if (!ageExists)
    {
        // Generate a random age between 1000 and 10000
        targetAge = Random.Range(1000, 10000);
        ageInputField.text = targetAge.ToString();
        PlayerPrefs.SetInt("AgePass", targetAge);
    }
    ...
    public void AddData()
    {
        ...
        string breed = breedInputField.text;
        ...
        int targetAge = GenerateUniqueRandomAge();
        ageInputField.text = targetAge.ToString();
        PlayerPrefs.SetInt("AgePass", targetAge);
        ...
        breedInputField.text = "";
        SetDropdownValue(genderDropdown, "");
        SetDropdownValue(obtainDropdown, "");
        //SetDropdownValue(obtainDropdown, "stageMaleDropdown");
    }

    private int GenerateUniqueRandomAge()
{
    int newAge;
    bool ageExists;
    ...
    {
        // Generate a random age between 1000 and 10000
        newAge = Random.Range(1000, 10000);
        ageExists = dataList.Exists(goatData => goatData.age == newAge);
    }
    while (ageExists);
   ...
   public void UpdateDataByAge(int targetAge)
    {
        ...
        if (targetData != null)
        {
            ...
            targetData.breed = breedInputField.text;
    ...
    public void UpdateDataOfStatus(int targetAge)
    {
        ...
        if (targetData != null)
        {
           ...
           StatusText.text  = StatusDropdown.options[StatusDropdown.value].text;
    ...
    public void UpdateDataOfStageG(int targetAge)
    {
        ...
        if (targetData != null)
        {
            ...
            {
                 targetData.stageG = stageMaleDropdown.options[stageMaleDropdown.value].text;
                 stageText.text = stageMaleDropdown.options[stageMaleDropdown.value].text;

                if (targetData.stageG == "Kid")
                {
                    targetData.statusG = "Non-Lactating"; // Set the appropriate status for KID stage
                    StatusText.text  = "Non-Lactating";
                }
            }
            else
            {
                  ...
                  targetData.stageG = stageFemaleDropdown.options[stageFemaleDropdown.value].text;
                  stageText.text = stageFemaleDropdown.options[stageFemaleDropdown.value].text;
                if (targetData.stageG == "Kid")
                {
                    targetData.statusG = "Non-Lactating"; // Set the appropriate status for KID stage
                     StatusText.text  = "Non-Lactating";
   ...
   private void DisplayDataByAge(int targetAge)
    {
        ...
        if (targetItem != null)
        {
            ...
            breedInputField.text = targetItem.breed;
            ...
            if (targetItem.gender == "Male")
            {
                SetDropdownValue(stageMaleDropdown, targetItem.stageG);
            }
            else
            {
                SetDropdownValue(stageFemaleDropdown, targetItem.stageG);
            }
        }
        else
        {
